...
While Zenodo is not the only option, we prefer it because it is free,
supports up to 60GB per record, and adheres to the FAIR standards.

Each item published on Zenodo is assigned two DOIs: one at the project
level and one for each version, making it easy to reference and cite in
academic work. A unique feature of Zenodo is its support for versioning,
allowing researchers to update their work over time, while earlier
versions remain accessible. This ensures transparency and
reproducibility, crucial in scientific research.

Zenodo’s metadata can cross-reference other datasets and code records,
promoting greater collaboration and integration across projects. The
zen4R package [@blondel2024] enhances the integration of R with Zenodo
by providing tools to programmatically interact with Zenodo’s API
...
::: notes
-   Zenodo provides a repository for long-term archiving of code and datasets, ensuring accessibility.

-   Items are assigned permanent DOIs at both project and version levels.
...
-   Integration with GitHub allows automatic archiving of new code
    releases, ensuring preservation
:::
